...
#   curl -sL https://raw.githubusercontent.com/nipegun/p-scripts/refs/heads/master/MVs/ChromeOSFlex-Instalar.sh | bash
#
# Ejecución remota como root con parámetros (Para indicar almacenamiento específico e ID de máquina virtual deseado):
#   curl -sL https://raw.githubusercontent.com/nipegun/p-scripts/refs/heads/master/MVs/ChromeOSFlex-Instalar.sh | bash -s "NombreDelAlmacenamiento" "IDDeLaMV"
#
...
# Definir id de la nueva máquina virtual
  vIdDeLaNuevaMV="${2:-}"
# Definir la ruta a la carpeta de descargas por defecto del usuario
  vRutaACarpetaDeDescargas=$(xdg-user-dir DOWNLOAD)
...
# Descargar la última versión del recovery de ChromeOSFlex
  ...
  mv -vf "$vRutaACarpetaDeDescargas"/chromeos-flex-latest.bin /tmp/chromeos-flex-latest.bin
...
# Notificar fin de ejecución del script
  ...
  echo '        - Ve a: "Boot Maintenance Manager" >> "Boot From File" >> "EFI-SYSTEM"'
  echo "        - Selecciona el archivo /efi/boot/bootx64.efi"
  echo ""
  echo '      Al finalizar la instalación apaga la máquina virtual y borra la linea que empieza por "args"'
  echo "        en el archivo /etc/pve/qemu-server/"$vIdDeLaNuevaMV".conf"
  echo ""
