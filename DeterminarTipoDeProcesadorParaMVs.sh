...
vFlags=$(cat /proc/cpuinfo | grep flags | head -n 1 | cut -d: -f2 | sed 's- -\n-g' | sort | tr '\n' ' ')

vSupports_v2='awk     "/cx16/&&/lahf_lm/&&/pni/&&/popcnt/&&/sse4_1/&&/sse4_2/&&/ssse3/        {found=1} END {exit !found}"'
...
echo ""
echo "$vFlags" | eval $vSupports_v2     || exit 2 && echo "    x86-64-v2     (Flags de Intel Nehalem y AMD Opteron_G3, o superiores)"
echo "$vFlags" | eval $vSupports_v2_AES || exit 3 && echo "    x86-64-v2-AES (Flags de Intel Westmere y AMD Opteron_G4, o superiores)"
echo "$vFlags" | eval $vSupports_v3     || exit 4 && echo "    x86-64-v3     (Flags de Intel Broadwell y AMD EPYC, o superiores)"
echo "$vFlags" | eval $vSupports_v4     || exit 5 && echo "    x86-64-v4     (Flags de Intel Skylake y AMD EPYC v4 Genoa, o superiores)"
...
vFlags-v3=$(    echo $vFlags | grep -E --color 'cx16|lahf_lm|pni|popcnt|sse4_1|sse4_2|ssse3| aes |avx|avx2|bmi1|bmi2|f16c|fma|movbe|xsave')
vFlags-v4=$(    echo $vFlags | grep -E --color 'cx16|lahf_lm|pni|popcnt|sse4_1|sse4_2|ssse3| aes |avx|avx2|bmi1|bmi2|f16c|fma|movbe|xsave|avx512f|avx512bw|avx512cd|avx512dq|avx512vl')
